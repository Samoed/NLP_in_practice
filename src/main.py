...
def interface() -> gr.Interface:
    with gr.Blocks(title="Resume suggestions") as block:
        with gr.Row():
            ...
            vacancy = gr.TextArea(
                lines=20, label="Vacancy", placeholder="Paste vacancy description here", value=VACANCY
            )
        with gr.Row():
            iconic_resume = gr.TextArea(
                lines=20, label="Iconic Resume", placeholder="Iconic resume will appear here", value=ICONIC_RESUME
            )
